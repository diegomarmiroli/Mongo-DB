...
const { connectToCollection, generateId, disconnect } = require("../connection_db");
...
app.delete("/api/v1/muebles/:codigo", async (req, res) => {
    try {
        ...
        if (!(await conn.findOne({ codigo: Number(codigo) }))) return res.status(400).send(JSON.stringify({ message: "El código no corresponde a un mueble registrado" }));
